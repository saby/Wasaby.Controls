      ...
      _beforeMount: function() {
         this._private = {
            getPanelSource: function(key) {
               var data = [];
               if (key) {
                  data = data.concat(this.PANEL_ITEMS_FIRST);
               }
               if (key === 2) {
                  data = data.concat(this.PANEL_ITEMS_SECOND);
               }
         ...
         this._viewSource = new MemorySource({
            idProperty: 'id',
            data: this.DEMO_ITEMS
         });
         this._sourceConfig = new MemorySource({
            idProperty: 'id',
            data: this.DEMO_ITEMS
         });
         this._source = this._private.getPanelSource(1);
      },
      sourceChange: function(e, key) {
         this._sourceNumber = key;
         this._source = this._private.getPanelSource(key);
      },
      _reset: function() {
         this._eventName = '';
      },
