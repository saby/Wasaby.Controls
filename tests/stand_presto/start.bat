...
set COPY_MODULS="ws" "%MODULES_FROM_PATH%\Бизнес-логика" "%MODULES_FROM_PATH%\BL Core" "%MODULES_FROM_PATH%\Работа с БД" "%MODULES_FROM_PATH%\Python" "%MODULES_FROM_PATH%\Python Core" "%MODULES_FROM_PATH%\Логирование" "www\service\sbis_root" "%MODULES_FROM_PATH%\Пользовательские настройки" "%MODULES_FROM_PATH%\Пользователь" "client\SBIS3.CONTROLS"
...
@echo Копируем Модуль - SBIS3.CONTROLS
...
py.exe -3 CreateSimpleCloud.py -i "%MODULES_TO%" -o "%MODULES_TO%\Тест.s3cld" --global-parameters "0" --host "%host%" --port "%port%" --dbname "%dbname%" --username "%username%" --password "%password%" --resources-root "/resources/" --ws-root "/ws/" --site-root "%SITE_ROOT%" --start-window-module "intest" --start-window "./index.xaml" "Бизнес-логика" "Работа с БД" "ИнТест" "Python" "Python Core" "Логирование" "intest" "Тема Скрепка" "BL Core" "Пользовательские настройки" "Пользователь" "SBIS3.ENGINE" "SBIS3.CONTROLS"
popd
