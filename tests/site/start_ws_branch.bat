...
set MODULES_FROM=%PLATFORMT_DIR%\www\service\Модули
set MODULES_TO=%DIR_SITE%\service\Модули
...
robocopy "%PLATFORMT_DIR%\test\sbis\Модули\i18n"	"%DIR_SITE%\service" sbis-lang300.dll 
...
robocopy "%MODULES_FROM%\Бизнес-логика" "%MODULES_TO%\Бизнес-логика" /E /MT /PURGE /LOG:"%LOG_SPACE%\copy_moduls_%VERSION%_verstka.log"
@if errorlevel 4 (
	@echo "Возникли проблемы при копировании модулей"
	exit /b 1
)
@echo Копируем Модуль - Работа с БД
robocopy "%MODULES_FROM%\Работа с БД" "%MODULES_TO%\Работа с БД" /E /MT /PURGE /LOG+:"%LOG_SPACE%\copy_moduls_%VERSION%_verstka.log"
@if errorlevel 4 (
	@echo "Возникли проблемы при копировании модулей"
	exit /b 1
)
...
robocopy "%MODULES_FROM%\Логирование" "%MODULES_TO%\Логирование" /E /MT /PURGE /LOG+:"%LOG_SPACE%\copy_moduls_%VERSION%_verstka.log"
@if errorlevel 4 (
	@echo "Возникли проблемы при копировании модулей"
	exit /b 1
)
@echo Копируем Модуль - Пользовательские настройки
robocopy "%MODULES_FROM%\Пользовательские настройки" "%MODULES_TO%\Пользовательские настройки" /E /MT /PURGE /LOG+:"%LOG_SPACE%\copy_moduls_%VERSION%_verstka.log"
@if errorlevel 4 (
	@echo "Возникли проблемы при копировании модулей"
	exit /b 1
)
@echo Копируем Модуль - Пользователь
robocopy "%MODULES_FROM%\Пользователь" "%MODULES_TO%\Пользователь" /E /MT /PURGE /LOG+:"%LOG_SPACE%\copy_moduls_%VERSION%_verstka.log"
@if errorlevel 4 (
	@echo "Возникли проблемы при копировании модулей"
	exit /b 1
)
