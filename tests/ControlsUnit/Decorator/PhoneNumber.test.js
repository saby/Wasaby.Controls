   ...
   function(decorator) {
      ...
      describe('Controls.Decorator.PhoneNumber', function() {
         ...
         it('Empty number', function() {
            result = decorator.PhoneNumber._private.formatPhone('');
            assert.equal(result, '');
         });

         it('Only letters in number', function() {
            result = decorator.PhoneNumber._private.formatPhone('hhhhh');
            assert.equal(result, '');
         });

         it('City Number with 8', function() {
            result = decorator.PhoneNumber._private.formatPhone('84942228771');
            assert.equal(result, '+7(4942) 22-87-71');
         });

         it('City Number with +7', function() {
            result = decorator.PhoneNumber._private.formatPhone('+74942228772');
            assert.equal(result, '+7(4942) 22-87-72');
         });

         it('Country Number with 8', function() {
            result = decorator.PhoneNumber._private.formatPhone('84943121750');
            assert.equal(result, '+7(49431) 2-17-50');
         });

         it('Country Number with +7', function() {
            result = decorator.PhoneNumber._private.formatPhone('+74943121751');
            assert.equal(result, '+7(49431) 2-17-51');
         });

         it('Country Number with wrong hyphens', function() {
            result = decorator.PhoneNumber._private.formatPhone('+7494-3121-752');
            assert.equal(result, '+7(49431) 2-17-52');
         });

         it('City Number with additional number', function() {
            result = decorator.PhoneNumber._private.formatPhone('84942228771112');
