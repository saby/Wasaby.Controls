...
define([
   ...
   describe('Controls.List.BaseControl', function() {
      ...
      describe('EditInPlace', function() {
         ...
         it('_onBeforeBeginEdit', function() {
            var opt = {
               test: 'test'
            };
            var newOpt = {
            ...
            ctrl._notify = function(e, options) {
               assert.equal('beforeBeginEdit', e);
               assert.equal(options[0], opt);
               return newOpt;
            };
            var result = ctrl._onBeforeBeginEdit({}, opt);
         ...
         it('_onBeforeEndEdit', function() {
            ...
            ctrl._children = {itemActions: {updateItemActions: function() {}}};
            ctrl._notify = function(e, options) {
               assert.equal('beforeEndEdit', e);
               assert.equal(options[0], opt);
               assert.isTrue(options[1]);
               assert.isTrue(options[2]);
               return newOpt;
            };
            var result = ctrl._onBeforeEndEdit({}, opt, true, true);
            assert.equal(result, newOpt);
         });
         ...
         it('_onBeforeEndEdit', function() {
            ...
            var result = ctrl._onBeforeEndEdit({}, opt);
            assert.equal(result, newOpt);
         });
