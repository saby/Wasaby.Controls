...
node('controls') {
    ...
    buildDiscarder(
        ...
        parameters([
            ...
            booleanParam(defaultValue: true, description: "Запуск интеграционных тестов по изменениям. Список формируется на основе coverage существующих тестов по ws, engine, controls, ws-data", name: 'run_int'),
