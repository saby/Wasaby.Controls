                    ...
                    stage("Инт.тесты"){
                        if ( inte || all_inte && smoke_result ){
                            ...
                            dir("./controls/tests/int"){
                                ...
                                sh """
                                source /home/sbis/venv_for_test/bin/activate
                                python start_tests.py ${tests_for_run} ${run_test_fail} ${skip_tests_int} --SERVER_ADDRESS ${server_address} --STREAMS_NUMBER ${stream_number} --JENKINS_CONTROL_ADDRESS jenkins-control.tensor.ru --RECURSIVE_SEARCH True
                                deactivate
                                """
                            }
                        }
                    ...
                    stage("Рег.тесты"){
                        if ( regr && smoke_result){
                            ...
                            dir("./controls/tests/reg"){
                                sh """
                                    source /home/sbis/venv_for_test/bin/activate
                                    python start_tests.py ${run_test_fail} ${skip_tests_reg} --SERVER_ADDRESS ${server_address} --STREAMS_NUMBER ${stream_number} --JENKINS_CONTROL_ADDRESS jenkins-control.tensor.ru --RECURSIVE_SEARCH True
                                    deactivate
                                """
                            }
                        }
