...
node('controls') {
    ...
    buildDiscarder(
        ...
        parameters([
            ...
            booleanParam(defaultValue: true, description: "SKIP тестов, которые падают в RC на текущий момент", name: 'skip'),
    ...
    ws(workspace) {
        ...
        parallel(
            ...
            int_reg: {
                    ...
                    if ( skip ) {
                         skip_tests_int = "--FAIL_FROM_RC '(int-ff) ${version} controls' --JENKINS_CONTROL_ADDRESS jenkins-control.tensor.ru"
                         skip_tests_reg = "--FAIL_FROM_RC '(reg-ff) ${version} controls' --JENKINS_CONTROL_ADDRESS jenkins-control.tensor.ru"
                    }
                    parallel (
                        int_test: {
                            stage("Инт.тесты"){
