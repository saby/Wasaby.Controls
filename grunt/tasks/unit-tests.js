...
module.exports = function(grunt) {
    ...
    grunt.registerTask('tests-setup-packages', function() {
        var path = require('path'),
            DriverProvider = require(path.join(process.cwd(), 'test/lib/webdriver')).Provider;
    ...
    grunt.registerTask('tests-run-webdriver[main]', function() {
        ...
        runTest(this.async(), 'unit-webdriver.run');
    });

    grunt.registerTask('tests-run-webdriver', [
    ...
    grunt.registerTask('tests-run-isolated[main]', function() {
        grunt.task.requires('js');
        runTest(
            this.async(),
            'unit-isolated.run',
    ...
    grunt.registerTask('tests-run-isolated', [
        'js',
        'tests-run-isolated[main]'
    ]);

    grunt.registerTask('tests-run-coverage[main]', function() {
        grunt.task.requires('tests-install-packages');
        grunt.task.requires('tests-setup-packages');
        grunt.task.requires('js');
        grunt.task.requires('express:development');
    ...
    grunt.registerTask('tests-run-coverage', [
        'tests-install-packages',
        'tests-setup-packages',
        'tests-list-build',
        'js',
