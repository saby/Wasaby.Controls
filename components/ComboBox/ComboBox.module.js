   ...
   var ComboBox = TextBox.extend([PickerMixin, DSMixin, Selectable, DataBindMixin], /** @lends SBIS3.CONTROLS.ComboBox.prototype */{
      ...
      $protected: {
         ...
         _options: {
             ...
             * Шаблон отображения элемента - это отдельный XHTML-файл.
             * Чтобы его можно было использовать, шаблон подключают в массив зависимостей компонента.
             * Внутри шаблона доступ к полям каждого элемента коллекции можно получить через инструкации шаблонизатора типа {{=it.item.get('ИмяПоля')}}.
             * @example
             ...
             *          ...
             *          'html!SBIS3.MyArea.MyComponent/itemTemplate'  // подлючаем шаблон в массив зависимостей компонента
             *       ],
             *       ...
             *    });
             * </pre>
