   ...
   function( // Подключенные в массиве зависимостей файлы будут доступны в следующих переменных
      ...
      var moduleClass = CompoundControl.extend({ // Наследуемся от базового компонента
         ...
         init: function() { // Инициализация компонента, здесь все дочерние компоненты готовы к использованию
            ...
            fcHelpers.message("В примере демонстрируется работа добавления по месту в иерархическом списке. Активация добавления производится двумя способами: либо кнопками из футера, либо из режима редактирования по месту на последнем элементе коллекции нажатием клавиши Enter.");
         }
      });
      return moduleClass;
   }
