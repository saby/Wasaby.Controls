...
var searchreplace = (function () {
  ...
  var $_dr4dlfisjedwglil = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_dr4dlfisjedwglil.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_8aw3r4iqjedwglie.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_8aw3r4iqjedwglie.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_8aw3r4iqjedwglie.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_8aw3r4iqjedwglie.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_8aw3r4iqjedwglie.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_8aw3r4iqjedwglie.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_8aw3r4iqjedwglie.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_8aw3r4iqjedwglie.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_8aw3r4iqjedwglie.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_8aw3r4iqjedwglie.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_8aw3r4iqjedwglie.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_8aw3r4iqjedwglie.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_8aw3r4iqjedwglie.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_8aw3r4iqjedwglie.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_8aw3r4iqjedwglie.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_ce4t8biujedwglj3 = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_ce4t8biujedwglj3.open(editor, currentIndexState);
    });
  };
  var $_akm1nritjedwgliz = { register: register };
  ...
  var $_2xpkjgivjedwglj9 = { register: register$1 };

  PluginManager.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_akm1nritjedwgliz.register(editor, currentIndexState);
    $_2xpkjgivjedwglj9.register(editor, currentIndexState);
    return $_50geehipjedwglia.get(editor, currentIndexState);
  });
  function Plugin () {
  }
