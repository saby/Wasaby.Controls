...
var toc = (function () {
  ...
  var $_ctgafkqyjedwgo5z = { create: create };

  var tocId = $_ctgafkqyjedwgo5z.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_79vd08qxjedwgo5x.getTocClass(editor);
    var headerTag = $_79vd08qxjedwgo5x.getTocHeader(editor);
    var selector = generateSelector($_79vd08qxjedwgo5x.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_79vd08qxjedwgo5x.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_79vd08qxjedwgo5x.getTocHeader(editor), I18n.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_e34zfzqtjedwgo5t.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_e34zfzqtjedwgo5t.updateToc(editor);
    });
  };
  var $_1rpkxlqsjedwgo5r = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_79vd08qxjedwgo5x.getTocClass(editor);
  ...
  var $_7w75r1qzjedwgo60 = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_e34zfzqtjedwgo5t.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_79vd08qxjedwgo5x.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_7tlddvr0jedwgo62 = { register: register$1 };

  PluginManager.add('toc', function (editor) {
    $_1rpkxlqsjedwgo5r.register(editor);
    $_7tlddvr0jedwgo62.register(editor);
    $_7w75r1qzjedwgo60.setup(editor);
  });
  function Plugin () {
  }
