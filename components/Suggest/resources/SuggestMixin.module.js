define('js!SBIS3.CONTROLS.SuggestMixin', [
    ...
    /**
     * Миксин автодополнения. Позволяет навесить функционал автодополнения на любой контрол.
     * Работает полностью через контекст.
     * Умеет обращаться с {@link list контролом списка сущностей}, реализующим заданный интерфейс.
     *
     * @remark
     * Обязательно требует миксины SBIS3.CONTROLS.PickerMixin и SBIS3.CONTROLS.DataBindMixin
     * @class SBIS3.CONTROLS.SuggestMixin
    ...
    var SuggestMixin = /** @lends SBIS3.CONTROLS.SuggestMixin.prototype */{
        ...
        $protected: {
            _options: {
                ...
                /**
                 * @cfg {$ws.proto.Control[]} Набор контролов, в которых отслеживается изменение данных
                 * @remark
                 * Контролы используются только для отслеживания получения ими фокуса.
                 * @group Data
                 */
                observableControls: [],
        ...
        _initList: function() {
            ...
            this.subscribeTo(this._list, 'onDataLoad', $.proxy(this._onListDataLoad, this));

            this.subscribeTo(this._list, 'onDrawItems', $.proxy(this._onListDrawItems, this));

            this.subscribeTo(this._list, 'onSelectedItemChange', $.proxy(function(eventObject, id) {
                this._onListItemSelect(id);
            }, this));

            this.subscribeTo(this._list, 'onSelectedItemsChange', $.proxy(function(eventObject, idArray) {
