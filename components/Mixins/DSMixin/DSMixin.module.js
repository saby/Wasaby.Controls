define('js!SBIS3.CONTROLS.DSMixin', [
   ...
   var DSMixin = /**@lends SBIS3.CONTROLS.DSMixin.prototype  */{
      ...
      $protected: {
         ...
         _options: {
            /**
             * @cfg {String} Определяет поле элемента коллекции, которое является его идентификатором.
             * @remark
             * Значения ключевого поля используются для однозначной идентификации каждого элемента коллекции.
             ...
             * @remark
             * В поле связи используется для отображения значений поля для всех выбранных элементов коллекции.
             * Данные задаются либо с помощью опции {@link items}, либо методом {@link setDataSource}.
             * Для поля связи метод {@link setDataSource} задает источник данных для {@link SBIS3.CONTROLS.SuggestMixin#autoShow отображения}
             * списка значений автодополнения.
             * @example
            ...
            dataSource: undefined,
              ...
              * Для иерархических структур при пейджинге по скроллу опция также задаёт количество элементов коллекции,
              * подгружаемых с источника данных кликом по кнопке "Ещё".
              * Необходимо учитывать, что в количество элементов коллекции считаются и узлы, и листья.
              * Т.е. подсчёт идёт относительно полностью развёрнутого представления данных.
              * Например, узел с тремя листьями - это 4 элемента.
              * @example
             ...
             * @property {Function} render Функция для дополнительной обработки HTML-контейнера группируемой записи.
             */
            /**
             * @cfg {GroupBy} Настраивает группировку записей.
             * @remark
             * Если задать только поле записи (field), то все записи будут просто сгруппированы по блокам с одинаковыми
             ...
             * Пример использования опции в {@link SBIS3.CONTROLS.FieldLink#dictionaries справочнике выбора записи}
             * для поля связи:
             * ![](/DSMixin02.png)
             * В примере для отображения данных использован компонент класса {@link SBIS3.CONTROLS.DataGridView} -
             * rонтрол, отображающий набор данных в виде таблицы с несколькими колонками.
             *
      ...
      /**
       * Метод перерисовки списка без повторного получения данных
       */
      redraw: function() {
         this._redraw();
      },
      ...
      /**
       * Установка группировки элементов. Если нужно, чтобы стандартная группировка для этого элемента не вызывалась -
