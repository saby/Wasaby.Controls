   ...
   var DSMixin = /**@lends SBIS3.CONTROLS.DSMixin.prototype  */{
      ...
      $protected: {
         ...
         _options: {
             ...
             * @remark
             * Опция позволяет настроить поле, значение которого будет использовано для отображения выбранных элементов
             * коллекции внутри контрола.
             * Выбранный элемент коллекции может содержать множество полей; опция определяет поле,
             * необходимое для визуального отображения.
             * Данные задаются либо с помощью опции {@link items}, либо методом {@link setDataSource}.
             * @example
             ...
             * @remark
             * Механизм группировки позволяет в списках любых типов создавать группы записей. Подробнее о группировках вы
             ...
             *       var isFolder = item ? item.get('Раздел@') : false; // проверяем признак: является ли текущая запись папкой в иерархической структуре коллекции
             *       if (isFolder) { // если текущая запись это папка, то производим изменение контейнера
             ...
             * @remark
             * Устанавливает текст, который будет отображаться при отсутствии данных после запроса к источнику, и в результате фильтрации.
