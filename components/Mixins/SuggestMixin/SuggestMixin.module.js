define('js!SBIS3.CONTROLS.SuggestMixin', [
   ...
   var SuggestMixin = /** @lends SBIS3.CONTROLS.SuggestMixin.prototype */{
      ...
      $protected: {
         _options: {
            /**
             * @cfg {Number} Задержка перед началом поиска. Значение задаётся в миллисекундах.
             * @example
             * <pre class="brush: xml">
            ...
            startChar: 3,
            /**
             * @cfg {Boolean} Устанавливает режим работы автодополнения, в котором оно выводит список всех возможных значений при переходе фокуса на контрол.
             * @variant true: режим включен, список значений отображается
             * @variant false: режим выключен, список значений не отображается
             * @remark
             * Подробнее о функционале автодополнения вы можете прочитать в описании к классу {@link SBIS3.CONTROLS.SuggestMixin}.
             * Режим автоматического отображения включен: при получении фокуса контролом отображается выпадающий блок со
             * списком значений, используемых для автодополнения.
             ...
             * Для настройки автодополнения в поле связи используется опция {@link text}, с помощью которой привязываем значение поля связи к полю в контексте.
             * Список с большим количеством значений будет отображен с полосой прокрутки. Отображение большого списка можно настроить,
             * добавив в настройки выпадающего блока {@link list} для контрола, в котором отображается список,
             * режим постраничной навигации {@link showPaging} и количество записей {@link pageSize}.
             * @see list
             * @see listFilter
             * @see text
             */
