      ...
      around : {
         ...
         _onCollectionAddMoveRemove: function(parentFnc, event, action, newItems, newItemsIndex) {
            ...
            if (this._options.viewMode == 'table') {
               ...
               //2. Если это удаление
               //3. Если это плоский список
               // тогда можно отрисовать как обычно
               // в остальных случаях полная перерисовка
               if (!cInstance.instanceOfMixin(this, 'SBIS3.CONTROLS/Mixins/TreeMixin') || ((lastItemsIndex == newItemsIndex) && !(cInstance.instanceOfModule(newItems[0], 'WS.Data/Display/TreeItem') && newItems[0].isNode()) && !this._redrawOnCollectionChange) || action === 'rm' || action === 'a') {
                  parentFnc.apply(this, args);
               }
               else {
