   ...
   var Browser = ControlsBrowser.extend( /** @lends SBIS3.Engine.Browser.prototype */{
      ...
      _calculatePathPosition: function(){
         var headerWidth = this._getHeaderWidth(),
            viewWidth = this.getContainer().width(),
         ...
         if (headerWidth <= (viewWidth / 3)) {
            ...
            path.width(viewWidth - headerWidth);
            //Установим ширину контейнера для хлебных крошек
            this._breadCrumbs.getContainer().width(Math.floor(viewWidth - headerWidth - this._backButton.getContainer().outerWidth(true)) - 20);
         }
         ...
         if (headerHeight > 24){
            if (isAboveHeadPath){
               ...
               var backButtonCont = this._backButton.getContainer();
               backButtonCont.css('top', Math.round(headerHeight / 2 - backButtonCont.height() / 2 + path.height()));
            } else {
      ...
      _getHeaderWidth: function(){
         ...
         return (this._container.width() / 4);
      },
