define('SBIS3.CONTROLS/TextBox', [
   ...
   var TextBox = TextBoxBase.extend(/** @lends SBIS3.CONTROLS/TextBox.prototype */ {
      ...
      $constructor: function() {
         ...
         /**
          * Аттрибут required используется без указания значения, любое значение игнорируется. Поэтому управленять поведением этого аттрибута
          * можно только с помощью его наличия/отсутствия на DOM элементе. Это значит, что исползовать его в шаблоне используя флаг нельзя.
          * Используем его в коде.
          * Аттрибут нужен только в ie, чтобы управлять видимостью нашего placeholder используя псевдо класс :invalid.
          */
         if (constants.browser.isIE) {
            this._inputField.attr('required', '');
         }
         this._inputField
