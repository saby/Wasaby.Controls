...
export default class StickyHeader extends Control<IStickyHeaderOptions> {
    ...
    protected _getStyle(options?: IStickyHeaderOptions): string {
        ...
        if (fixedPosition) {
            ...
            // При построении, прилипающий заголовок может рассчитывать стили вплоть до 150мс,
            // на это время он может оказаться ниже контента, т.к. не имеет z-index.
            // Ставим z-index меньше на 1, чем fixedZIndex, иначе, заголовки которые не должны застикаться,
            // находящиеся ниже в DOM, будут перекрывать собой застиканные заголовки.
