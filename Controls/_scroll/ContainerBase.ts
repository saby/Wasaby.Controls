...
export default class ContainerBase extends Control<IContainerBaseOptions> {
    ...
    _afterMount(): void {
        ...
        if (this._getContentType() === CONTENT_TYPE.regular) {
    ...
    _resizeObserverCallback(entries: any): void {
        ...
        for (const entry of entries) {
            if (entry.target === this._children.scrollContainer) {
                ...
                if (contentType === CONTENT_TYPE.regular) {
                    newState.scrollHeight = entry.contentRect.height;
                } else if (contentType === CONTENT_TYPE.height100percentAndCanScroll) {
    ...
    _getContentType(): CONTENT_TYPE {
        ...
        const classList = this._children.content.classList;
        const classNames = [
            'Hint-ListWrapper',
            'Wizard-Vertical-Container__content'
        ]
        if (classNames.some(className => classList.contains(className))) {
            return CONTENT_TYPE.height100percentAndCanScroll;
        }
        return contentType;
    }
