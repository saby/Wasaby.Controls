 ...
 * Если в результате запроса для этих узлов будут присланы дочерние элементы, то узлы останутся раскрытыми, иначе они будут свёрнуты. постраничная навигация сейчас передается только для корня.
 * Таким образом, для остальных узлов необходимо отдавать количество записей, соответствующее переданному в параметрах навигации
