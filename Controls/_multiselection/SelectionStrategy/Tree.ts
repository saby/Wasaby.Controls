...
export class TreeSelectionStrategy implements ISelectionStrategy {
      ...
      if (!this._isAllSelected(selection, this._rootId) || !hasMoreData) {
         ...
         for (let index = 0; index < selectedNodes.length; index++) {
            ...
            if (nodeSourceController?.hasMoreData('down')) {
                ...
                const deep = this._selectDescendants
                    || !this._selectDescendants && !this._selectAncestors && this._isAllSelectedInRoot(selection);
                countItemsSelectedInNode = this._getSelectedChildrenCount(nodeKey, selection, this._items, this._hierarchyRelation, deep);
            }
   ...
   private _getStateNode(itemId: TKey, initialState: boolean, selection: ISelection): boolean|null {
      ...
      for (let index = 0; index < children.length; index++) {
         ...
         const childId: TKey = child.getKey();
   ...
   private _getAllChildren(nodeId: TKey, items: RecordSet, hierarchyRelation: relation.Hierarchy): Model[] {
         ...
         children.forEach((childItem) => {
            if (selectedChildrenCount !== null) {
               ...
               if (!selection.excluded.includes(childId)) {
                  ...
                  if (this._isNode(childItem) && this._getChildren(childItem.getKey(), items, hierarchyRelation).length > 0 && deep !== false) {
   ...
   private _getChildren(nodeId: TKey, items: RecordSet, hierarchyRelation: relation.Hierarchy): Model[] {
