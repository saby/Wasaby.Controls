...
var
   _private = {
       ...
       * @remark
       * Данные берутся из поля ввода. В случае, когда по одному событию произойдет, изменение данных полем ввода, и завершение редактирования,
       * тогда контрол обработает устаревшие данные. Потому что вызов обработчиков события происходит синхронно.
       * Например, в поле ввода с опцией trim = true ввели "      ". По уходу фокуса будут вызваны обработчики
       * поля ввода и редактирования. Поле ввода удалит лишние пробелы, сообщит родителю об изменении данных.
       * Редактирование обработает данные. ТОЛЬКО ПОТОМ получит уведомление от поля ввода об изменении и обработает иго.
       * Данный метод решает описанную выше проблему. Контрол завершит редактирование асинхронно, на этот помент данные будут актуальны.
       */
