define('Controls/Popup/Templates/Notification/NotificationStyles', [],
   function() {}

   /**
    * @mixin Controls/Popup/Templates/Notification/NotificationStyles
    * @public
    * @css @border-width_NotificationBase Width of border.
    * @css @size_NotificationBase-shadow Size of shadow
    * @css @color_NotificationBase-shadow Color of shadow.
    * @css @min-height_NotificationBase Minimum height.
    * @css @min-width_NotificationBase Minimum width.
    * @css @background-color_NotificationBase Background color.
    *
    * @css @border-color_NotificationBase_danger Border color when options style is set to danger.
    * @css @border-color_NotificationBase_primary Border color when options style is set to primary.
    * @css @border-color_NotificationBase_done Border color when options style is set to done.
    * @css @border-color_NotificationBase_warning Border color when options style is set to warning.
    *
    * @css @width_NotificationBase-close-button Width of closeButton.
    * @css @spacing_NotificationBase-between-closeButton-border-right Spacing between close-button and border-right
    * @css @spacing_NotificationBase-between-closeButton-border-top Spacing between close-button and border-top
    * @css @spacing_NotificationBase-between-content-border Spacing between content and border.
    * @css @spacing_NotificationBase-between-icon-and-message Spacing between message and icon.
    * @css @spacing_NotificationBase-between-notofications-right Spacing between notifications right.
    * @css @spacing_NotificationBase-between-notofications-bottom Spacing between notifications bottom
    * @css @font-size_NotificationBase-message Font size of message.
    */
);
