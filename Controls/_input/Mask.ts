...
import {spaceToLongSpace} from 'Controls/_input/Mask/Space';
      ...
      /**
       * Поле ввода значения с заданным форматом. Формат определяется {@link mask маской}. Маска состоит из обычных и {@link formatMaskChars специальных символов}.
       * Обычный символ - символ для форматирования значения, введенного пользователем. Он всегда будет присутствовать в полностью заполненном поле в независимости от того, что ввел пользователь.
       * Специальный символ - символ заменяющийся на введенные пользователем символы. Например: d - Цифровой символ, l - Буквенный символ в нижнем регистре.
       ...
       * Для того, чтобы показать обычные символы в незаполненном поле, нужно настроить замену специальных символов на {@link replacer обычный символ}: " ", "_", "*" и т.п.
       * Контрол с изначально видимыми обычными символами имеет автоширину по контенту. Задать свою ширину можно через css.
