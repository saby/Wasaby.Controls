...
var
    _private = {
        ...
        isNeedToHighlight(item, dispProp, searchValue): boolean {
            const itemValue = item.get(dispProp);
    ...
    GridViewModel = BaseViewModel.extend({
        ...
        _nextModelVersion(notUpdatePrefixItemVersion: boolean): void {
        ...
        isStickyHeader(): boolean {
           return GridLayoutUtil.isFullGridSupport() && !!this._options.stickyHeader;
        },
        ...
        goToNextResultsColumn(): void {
            this._curResultsColumnIndex++;
        },

        isEndResultsColumn(): boolean {
        ...
        getItemDataByItem: function(dispItem) {
            ...
            if (current.isGroup) {
                current.groupResultsSpacingClass = ' controls-Grid__cell_spacingLastCol_' + ((current.itemPadding && current.itemPadding.right) ||  'default').toLowerCase() + `_theme-${this._options.theme}`;
                return current;
            }
            ...
            if (this._options.rowSeparatorVisibility) {
                current.rowSeparatorVisibility = (current.isFirstInGroup && !current.isInHiddenGroup && current.item !== self.getLastItem());
            } else {
        ...
        getCurrent() {
            const dispItem = this._model._display.at(this._model._curIndex);
            return this.getItemDataByItem(dispItem);
        },

        toggleGroup(group, state): void {
