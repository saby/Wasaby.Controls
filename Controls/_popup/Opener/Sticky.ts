 ...
 * @variant overflow Координаты точки позиционирования меняются (окно сдвигается относительно целевого элемента настолько,насколько не помещается в области видимости экрана, причем окно, возможно, будет перекрывать целевой элемент.) Ширина и высота окна не меняются.
 * @variant adaptive Координаты точки позиционирования ({@link Controls/_popup/Opener/Sticky#targetPoint targetPoint}) и выравнивание ({@link Controls/_popup/Opener/Sticky#direction direction}) меняются на противоположные. Если и в этом случае окно не влезает, выбирается тот способ позиционирования (изначальный или инвертируемый), при котором на экране помещается наибольшая часть контента. Ширина и высота окна не меняются.
 * @default adaptive
 */
