...
/// <amd-dependency path="css!Controls/File/ResourceGetter/PhotoCam/Dialog" />
...
import tmpl = require("tmpl!Controls/File/ResourceGetter/PhotoCam/Dialog");
...
let DialogAbstract = CompoundControl.extend({
    ...
    _getFiles(): Deferred<Array<Blob>> {
        return this._getImage().addCallback((blob) => {
            if(!blob) {
     ...
     * @private
     */
    _prepareFile(image: File | Blob | string) {
