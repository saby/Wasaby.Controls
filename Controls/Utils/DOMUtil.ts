...
export class DOMUtil {
    ...
    /**
     * Вычисляет горизонтальный размер ещё не отрисованного блока с css классом, переданном во втором аргументе
     * Создаёт HTML контейнер с содержимым content и измеряет его гориз. размер.
     * @param content CSS класс элемента который необходимо измерить до отрисовки
     * @param blockClass CSS класс элемента который необходимо измерить до отрисовки
     * @param considerMargins Учитывать ли margin left/right
     */
    static getBlockWidth(
        content: string,
        blockClass: string,
        ...
        measurer.style.top = '-10000px';
        document.body.appendChild(measurer);
        const styles = window.getComputedStyle(measurer);
        blockSize = measurer.clientWidth +
            (considerMargins ? parseFloat(styles.marginLeft) + parseFloat(styles.marginRight) : 0);
        document.body.removeChild(measurer);
        return blockSize;
    }
