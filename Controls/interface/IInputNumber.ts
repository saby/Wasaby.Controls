 ...
 * @default 0
 * @remark
 * Если вы не обновите параметр value, то не сможете ничего ввести в поле. Необходимо подписаться на событие _valueChanged и обновить значение, передаваемое элементу управления. Чтобы сделать его проще, вы можете использовать синтаксис связывания.
 * Работая с типом числа можно столкнуться с проблемой потери точности. Например, номер 99999999999999999999 не существует. Он представлен как 100000000000000000000. Дело в том, что число в js представлено {@link http://www.softelectro.ru/ieee754.html IEEE 754} стандартом. Чтобы избежать этой проблемы, необходимо работать со строковым типом.
 * @example
 * В этом примере значение _input в состоянии элемента управления привязывается к значению поля ввода. В любое время жизненного цикла элемента управления значение _input будет содержать текущее значение поля ввода.
 * <pre>
...
/**
 * @event Возникает при изменении значения поля.
 ...
 * @remark
 * Это событие должно использоваться для реагирования на изменения, вносимые Пользователем в поле. Значение, возвращаемое в событии, не вставляется в элемент управления, если не передать его обратно в поле в качестве опции. Обычно вы бы вместо того, чтобы использовать синтаксис связывания. Пример ниже показывает разницу.
 * @example
 * В этом примере мы покажем, как можно 'привязать' значение элемента управления поле. В первом поле, мы делаем это вручную, используя событие valueChanged. Во втором поле используется нотация привязки. Оба поля в этом примере будут иметь одинаковое поведение.
 * <pre>
 ...
 * @param {String} displayValue Отображаемое значение в поле.
 * @remark
 * Это событие можно использовать в качестве триггера для проверки поля или отправки введенных данных в какой-либо другой элемент управления.
 * @example
 * В этом примере мы подписываемся на событие Input Completed и сохраняем значение поля в базе данных.
 * <pre>
