   ...
   var _private = {
      ...
      checkOverflow: function(popupCfg, targetCoords, position, direction) {
         ...
         if (position.hasOwnProperty(isHorizontal ? 'right' : 'bottom')) {
            ...
            let res = popupCfg.sizes[isHorizontal ? 'width' : 'height'] - (_private.getTargetCoords(popupCfg, targetCoords, isHorizontal ? 'right' : 'bottom', direction) - targetCoords[isHorizontal ? 'leftScroll' : 'topScroll']);
            if (!isHorizontal && popupCfg.targetPoint && popupCfg.targetPoint.vertical === 'bottom') {
               res += targetCoords.height;
            }
            return res;
         }
      ...
      setMaxSizes: function(popupCfg, position) {
         ...
         if (popupCfg.config.maxHeight) {
            ...
            if (position.top) {
               verticalScroll = _private.getVisualViewport().pageTop;
            } else {
               verticalScroll = _private.getBody().height - _private.getViewportHeight() -
                   _private.getVisualViewport().pageTop;
            }
