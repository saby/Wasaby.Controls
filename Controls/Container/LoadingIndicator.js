define('Controls/Container/LoadingIndicator', [
    ...
    *    -  id - defines the unique id of showing request (By default use autogenerated id),
    *    -  isGlobal - global or not (If not setted, by default use value of similar control option)
    *    -  message - message of indicator (If not setted, by default use value of similar control option)
    *    -  scroll - add gradient of indicator's background (If not setted, by default use value of similar control option)
    *    -  small - size of indicator (If not setted, by default use value of similar control option)
    *    -  overlay - setting of indicator's overlay (If not setted, by default use value of similar control option)
      ...
      toggleIndicator: function(isLoading) {
         ...
         if (this._isPreloading) {
            ...
            if (isLoadingStateChanged) {
               ...
               var delay = this.delay === 0 ? this.delay : (this.delay || this._delay);
      ...
      _toggleIndicator: function(visible, config, force) {
         ...
         if (visible) {
            if (force) {
               ...
               var delay = config.delay === 0 ? config.delay : (config.delay || this._delay);
               this.delayTimeout = setTimeout(function() {
                  this.isLoading = true;
                  this._updateProperties(config);
                  this._forceUpdate();
