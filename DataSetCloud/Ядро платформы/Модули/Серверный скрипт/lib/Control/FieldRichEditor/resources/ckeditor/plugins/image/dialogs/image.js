...
( function() {
	var imageDialog = function( editor, dialogType ) {
         ...
         var deleteSelectedImage = function() {
               if (this.currentImageUpdloaded) {
                  $.ajax({
            ...
            resizable: CKEDITOR.DIALOG_RESIZE_NONE,
				onShow: function() {
               if (this.wsInputFile instanceof $) {
                  this.wsInputFile.val('');
               }
               if (this.wsRequestResult instanceof $) {
                  this.wsRequestResult.html('');
               }
            ...
            onCancel: function() {
               //Если есть активный запрос на передачу изображения - прерываем его
               if (this.wsLastXHR instanceof XMLHttpRequest) {
                  this.wsLastXHR.abort();
               }
               //Если файл был сохранен на сервере - удаляем его
               deleteSelectedImage.call(this);
            },
				onOk: function() {
					// Edit existing Image.
					if ( this.imageEditMode ) {
				...
				contents: [
					...
					elements: [
							...
							children: [
                        {
                           ...
                           id: 'selectFileButton',
                           label: 'Выбрать файл',
                           hidden: $ws._const.browser.isIE8 || $ws._const.browser.isIE9,
                           onClick: function() {
                              this.getDialog().wsInputFile.trigger('click');
                           }
                        },
                        {
                           type: 'html',
                           html: '<div class="ws-field-rich-editor ws-loading-indicator ws-invisible">\
                                    <div class="ws-loading-indicator-image"></div>\
                                    <div class="ws-loading-indicator-text">Идёт загрузка изображения...</div>\
                                 </div>',
                           onLoad: function() {
                              this.getDialog().wsLoadingIndicator = $(this.getElement().$);
                           }
                        },
								{
								type: 'html',
								style: 'display:block',
                        html: '<input type="file" name="id" class="ws-field-rich-editor ws-upload-file-browse" accept="image/*"></input>',
                        onLoad: function() {
                           var input = $(this.getElement().$),
                               self = this,
                               dialog = self.getDialog();
                           dialog.wsInputFile = input;
                           input.change(function() { //При выборе файла создаем XMLHttpRequest
                              //Если ранее другое изображение было загружено на сервер - удаляем его
                              if (dialog.currentImageUpdloaded) {
                                 deleteSelectedImage.call(dialog);
                              }
                              var xhr = new XMLHttpRequest(),
                                  file = this.files[0],
                                  filePath = '/cstorage/img?id=' + $ws.helpers.createGUID(); //Генерируем псевдо-UID (подробности в описании функции "createGUID")
                              //Проверяем, является ли выбранный файл картинкой (хромой IE не выставляет фильтр в диалоге выбора файла)
                              if (file.type.indexOf('image')!==0) {
                                 //Если это так, то ругаемся в строку результатов и выходим
                                 dialog.wsRequestResult.html('Выбранный файл не является изображением.');
                                 dialog.wsRequestResult.addClass('ws-image-post-error');
                                 return;
                              }
                              //Подготавливаем юзерский интерфейс
                              dialog.getContentElement("info", "selectFileButton").disable();
                              dialog.wsRequestResult.html('');
                              dialog.wsLoadingIndicator.removeClass('ws-invisible');
                              //Запоминаем запрос (чтобы можно было прервать при закрытии диалога)
                              dialog.wsLastXHR = xhr;
                              //Подготавливаем XHR
                              xhr.open('PUT', filePath, true);
                              xhr.setRequestHeader('Content-Disposition', 'attachment');
                              xhr.setRequestHeader('Content-Type', file.type);
                              xhr.onreadystatechange = function() { //Выполняется по завершению запроса
                                 //Скрываем индикатор загрузки и активируем кнопку выбора файла
                                 dialog.wsLoadingIndicator.addClass('ws-invisible');
                                 dialog.getContentElement("info", "selectFileButton").enable();
                                 //Проверяем, как закончился запрос
                                 if (xhr.readyState === 4) {
                                    if (xhr.status !== 201) { //Если полученный статус не 201, то картинка на сервере не сохранилась
                                       switch (xhr.status) { //Ругаемся в зависимости от статуса запроса
                                          case 413:
                                             msg = 'Размер выбранного изображения превышает максимально допустимый.'
                                             break;
                                          default:
                                             msg = 'Не удалось загрузить изображение. Повторите попытку.'
                                       }
                                       dialog.setValueOf('info', 'txtUrl', '');
                                       input.val('');
                                    } else {
                                       msg = 'Изображение успешно загружено.'
                                       dialog.setValueOf('info', 'txtUrl', filePath);
                                    }
                                 } else {
                                    msg = 'Не удалось загрузить изображение. Повторите попытку.'
                                    dialog.setValueOf('info', 'txtUrl', '');
                                 ...
                                 dialog.wsRequestResult.html(msg);
                                 dialog.wsRequestResult.toggleClass('ws-image-post-error', xhr.status !== 201 || xhr.readyState !== 4);
                                 dialog.wsLastXHR = undefined;
                                 dialog.currentImageUpdloaded = true; //Флаг, означающий что файл был успешно загружен на сервер
                              };
                              xhr.send(file);
                           });
                        }
					...
					},
                  {
                     id: 'requestResult',
                     type: 'html',
                     html: '<div class="ws-field-rich-editor request-result"></div>',
                     onLoad: function() {
                        this.getDialog().wsRequestResult = $(this.getElement().$);
                     },
                     onShow: function() {
                        //Если это 8 или 9 Internet Explore - то выводим соответствующее сообщение
                        if ($ws._const.browser.isIE8 || $ws._const.browser.isIE9) {
                           var element = $(this.getElement().$);
                           element.addClass('ws-image-post-error');
                           element.html('Данный браузер не поддерживает загрузку изображений.');
                        }
                     }
                  },
