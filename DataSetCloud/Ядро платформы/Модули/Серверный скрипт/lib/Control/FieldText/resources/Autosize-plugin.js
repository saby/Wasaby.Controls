...
*/
define('js!SBIS3.CORE.FieldText/resources/Autosize-plugin', [], function () {
   (function ($) {
      var
         defaults = {
         ...
         return this.each(function () {
            ...
            if (firstRunned) {
               var savedValue = ta.value,
                   maxLinesCount = $ta.data('maxLinesCount'),
                   minLinesCount = $ta.data('minLinesCount');
               ta.value = 't';
               initMirror();
               linesHeigth = calculateHeight();
               ta.value = savedValue;
               minHeight = (linesHeigth * (minLinesCount > 0 ? minLinesCount : 1)) + defaults.additionalHeight;
               firstRunned = false;
            }
            ...
            function calculateHeight() {
               ...
               if ('getComputedStyle' in window) {
                  ...
                  width = ta.getBoundingClientRect().width || $(mirror).width();
            ...
            function adjust() {
               ...
               if (maxLinesCount > 0) {
                  maxHeight = (maxLinesCount * linesHeigth) + defaults.additionalHeight;
               } else {
                  maxHeight = parseInt($ta.css('maxHeight'), 10);
               }
               minHeight = (linesHeigth * (minLinesCount > 0 ? minLinesCount : 1)) + defaults.additionalHeight;
               height = calculateHeight() + defaults.additionalHeight;
               ...
               if (original !== height) {
                  $(ta).trigger('autosize.actual.resize');
                  ta.style.height = height + 'px';
                  if (callback) {
                     options.callback.call(ta, ta);
                  }
               }
               if (needScroll) { //Если показали скролл первый раз, то прокручиваем текст до конца
                  ta.scrollTop = 9e4;
               }
            }
            ...
            if ('onpropertychange' in ta) {
               if ('oninput' in ta) {
                  ...
                  $ta.bind('input.autosize keyup.autosize', adjust);
               } else {
                  // IE7 / IE8
                  $ta.bind('propertychange.autosize', function () {
            ...
            if (options.resizeDelay !== false) {
               $(window).bind('resize.autosize', resize);
            }

            // Event for manual triggering if needed.
            // Should only be needed when the value of the textarea is changed through JavaScript rather than user input.
            $ta.bind('autosize.resize', adjust);

            // Event for manual triggering that also forces the styles to update as well.
            // Should only be needed if one of typography styles of the textarea change, and the textarea is already the target of the adjust method.
            $ta.bind('autosize.resizeIncludeStyle', function () {
               mirrored = null;
               adjust();
            });

            $ta.bind('autosize.destroy', function () {
               mirrored = null;
               clearTimeout(timeout);
