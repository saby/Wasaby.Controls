/**
 * BigNumerics for JavaScript
 * Forked from http://github.com/jtobey/javascript-bignum
 * Wrapped By daftcoder
 *
 * Examples:
 * var a = new BigNum()    // is       0
 * a = a.add(123456)       // is       123456
 * a = a.add(a)            // is       246912
 * a = a.divide(7)         // is     ~ 35273.1428571429
 * a + ''                  // returns "35273.14"
 * a = a.add('4/10')       // is     ~ 35273.5428571429
 * a.toString(0)           // returns "35274"
 *
 * a = a.divide( a.multiply( a.sub( 1 ) ) )
 * a.toString(10)          // returns "0.0000283507"
 */
bigCalc = (function() {
   {
      /*
      ...
      if (!Array.prototype.map) {
         Array.map = function(self, fun /*, thisp*/) {
      ...
      if(!Array.indexOf) { // copypaste from core.js
      ...
      if('$ws' in global && $ws.helpers && $ws.helpers.forEach) {
         forEachMethod = $ws.helpers.forEach;
      }
      else { // copypaste from core.js
         forEachMethod = function(obj, iterateCallback, context) {
         ...
         if (parts) {
            ...
            if (base == 10) {
               ...
               while (digits.length >= BigInteger.base_log10) {
                  d.push(parseInt(digits.splice(Math.max(0,digits.length-BigInteger.base_log10), BigInteger.base_log10).join(''), 10));
               }
            ...
            if (base === BigInteger.base) {
               return new BigInteger(Array.map(digits, Number).reverse(), sign);
            }
      ...
      var SchemeNumber = (function() {
         ...
         var DispatchJs = (function() {
            ...
            function makeContext(opts) {
               ...
               var Array_indexOf  = Array.indexOf;
         ...
         function PluginContainer(init) {
            ...
            var onChange = {
               ...
               fire: function(changes) {
                  ...
                  forEachMethod(listeners, notify);
               },
         ...
         function implementBigInteger(plugins, BigInteger) {
            ...
            var String_replace  = uncurry(g.String.prototype.replace);
            var Array_concat    = uncurry(g.Array.prototype.concat);
            var Array_map       = uncurry(g.Array.map);
         ...
         function defaultIntegerFactory(plugins) {
            ...
            var BgIntgr = BigInteger;

            /*            if (typeof require !== "undefined")
             BgIntgr = require("./biginteger").BigInteger;
             else
             BgIntgr = this.BigInteger;*/
            ...
            return implementBigInteger(plugins, BgIntgr);
         }
   ...
   /**
    * Переводит число в строку с учётом, что 1e-7 это 0.0000001
    * @param value
    * @returns {string}
    */
   function numberToString(value){
      var str = value + '';
      return str.indexOf('e') === -1 ? str : value.toFixed(20);
   }

   /**
    * Конвертирует строковое число в SchemeNumber.
    * Целое число отдаёт как есть.
    * Вещественное число превращает в обыкновенную дробь (для точности вычислений).
    */
   function convertToSchemeNumber(value) {
      var
         n = value.indexOf('.'),
         m;
      if(n!==-1) {
   ...
   /**
    * Парсит выражение.
    * Возвращает посчитанное выражение.
    * Используем снача преобразование в польскую префиксную, а затем выполняем операции.
    * @param str
    * @param {Array|Object} [args]
    * @return {SchemeNumber, String}
    */
   function parseExpression(str, args) {
      if(str && str._v) {
         return str._v
      }
      if(str && typeof str == 'string' && args) {
         str = str.replace(/%([a-z0-9а-я_]+)/ig, function(match, argnum){
            argnum = isNaN(Number(argnum)) ? argnum : (argnum - 1);
            if(argnum in args) {
               if(isNaN(Number(args[argnum]))) {
                  throw Error("Замена для аргумента " + match + " (" + args[argnum] + ") не явлется числом");
               } else {
                  return args[argnum];
               }
            } else {
               throw Error("Для аргумента " + match + " не предоставлена замена");
            }
         });
      }
      str = str || "0";
      ...
      function convertToPrefix(str){
         ...
         function parseNumber(){
            var res = str.match(/^\s*([+-]*)\s*([0-9]+(?:\.[0-9]+)*)\s*/);
            if(res) {
         ...
         function parseNotNumber(){
            var res = str.match(/^\s*(>=|<=|==|!=|[\(\)\+\-*/><=])\s*/);
            if(res) {
               str = str.substr(res[0].length);
               if(res[1] == '==') {
                  res[1] = '=';
         ...
         while(str.length) {
            ...
            if(tmp) {
               if(waitNum && tmp != '(') {
                  throw("Unparsable expression: expected number or expression at >\"" + str + '\".');
               }
               waitNum = true;
               if(tmp == '(') {
                  stack.push(tmp);
               ...
               while(stack.length && priorities[stack[stack.length-1]] >= priorities[tmp]) {
               ...
               if( tmp == ')' ) {
                  ...
                  if(stack.length && stack[stack.length-1] == '(') {
                     ...
                     throw("Unparsable expression: closing bracket without opening.");
                  }
               } else {
                  stack.push(tmp);
               }
            ...
            throw("Unparsable expression: at symbol >\"" + str + '\".');
         }
         while(stack.length && stack[stack.length-1] != '(') {
            result.push(stack.pop());
         }
         if(stack.length) {
            throw("Unparsable expression: opening bracket without closing.");
         }
         return result;
      }
      var
         queue = convertToPrefix(str),
         stack = [],
         i;
      for(i=0; i<queue.length; ++i) {
         if(priorities[queue[i]]) {
            if(stack.length < 2) {
               throw("Unparsable expression: incorrect queue in prefix notation (not enough operands) >\"[" + queue.join(',') + '][' + i + ']\".');
            }
            var
               y = stack.pop(),
               x = stack.pop();
            if( priorities[queue[i]] == 1 && i != queue.length -1 ) {
               throw("Unparsable expression: multiple logic operations.");
            }
            stack.push(innerBigCalc(x,queue[i],y));
         } else {
            stack.push(queue[i]);
         }
      }
      if(stack.length > 1) {
         throw("Unparsable expression: incorrect queue in prefix notation (too much operands) >\"[" + queue.join(',') + '][' + i + ']\".');
      }
      if(stack.length < 1) {
         throw("Unparsable expression: incorrect queue in prefix notation (not enough operands) >\"[" + queue.join(',') + '][' + i + ']\".');
      }
   ...
   function mySchemeNumberConstructor(value) {
      value = value || '0';
      if(typeof value ==  "number") {
         value = numberToString(value);
      }
      if(typeof value == "string") {
         return convertToSchemeNumber(value);
      } else if(value && value._v){
         return value._v;
      } else {
         return value;
      }
   ...
   function innerBigCalc(operand1, operation, operand2) {
      var
         bigNumReturnedOps = [
            '+',
            '-',
         ...
         allowedOps = bigNumReturnedOps.concat(booleanReturnedOps).concat(stringReturnedOps);

      if(Array.indexOf(allowedOps, operation) == -1) {
         var parseArgs = [].slice.apply(arguments), parseResult;
         parseArgs.shift();
         if(parseArgs.length == 1 && typeof parseArgs[0] == 'object')
            parseArgs = parseArgs[0];
         parseResult = parseExpression.apply(null, [ operand1, parseArgs ]);
         if(typeof(parseResult)=="boolean") {
            return parseResult;
         }
         return new MyBigNum(parseResult);
      }

      if(operation == '!=') {
         return !SchemeNumber.fn['='](mySchemeNumberConstructor(operand1), mySchemeNumberConstructor(operand2));
      } else if(Array.indexOf(bigNumReturnedOps, operation) != -1) {
         return new MyBigNum(SchemeNumber.fn[operation](mySchemeNumberConstructor(operand1), mySchemeNumberConstructor(operand2)));
      }
      else if(Array.indexOf(booleanReturnedOps, operation) != -1) {
         return SchemeNumber.fn[operation](mySchemeNumberConstructor(operand1), mySchemeNumberConstructor(operand2));
      }
      else if(Array.indexOf(stringReturnedOps, operation) != -1) {
         if(operation === 'toString') {
            return new MyBigNum(operand1).toString(operand2);
         } else {
            throw "Behaviour of operation " + operation + " should be defined in bignumLibrary";
         }
      }
      else {
         throw "bigNum operation \"" + operation + "\" is no allowed. Allowed operations are " + allowedOps.join(', ') + '. ';
      }
   }

   function MyBigNum(value) {
      this._v = mySchemeNumberConstructor(value);
   }

   MyBigNum.prototype.toString = function(precesion) {
      precesion = precesion !== undefined ? precesion : 2;
      return this._v.toFixed(precesion);
   };
